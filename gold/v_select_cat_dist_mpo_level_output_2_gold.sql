...
CREATE OR REPLACE SECURE VIEW GOLD.SECURE_V_CAT_DIST_MPO_LEVEL_OUTPUT_2_GOLD
COMMENT = 'This secure view provides the output at a category, district, and MPO level to be connected to Tableau for the dashboard. It includes authorized amounts from project details with LET and Cost overruns adjustments as well as targets, carryovers and change orders amounts.' 
AS
SELECT
    category,
    district_mpo_division,
...
GRANT SELECT ON VIEW UTP_DASHBOARD.GOLD.SECURE_V_CAT_DIST_MPO_LEVEL_OUTPUT_2_GOLD TO SHARE TTI_UTP_DASHBOARD_SHARE;